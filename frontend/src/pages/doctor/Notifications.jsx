  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        {unreadCount > 0 && (
          <button onClick={markAllAsRead} className="text-sm text-green-700 hover:underline font-medium">
            Mark all as read
          </button>
        )}
      </div>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl mb-6 w-fit">
        {["all", "unread", "read"].map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition capitalize ${filter === f ? "bg-white text-green-700 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
          >
            {f}
