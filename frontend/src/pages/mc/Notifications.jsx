  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        {unreadCount > 0 && (
          <button onClick={markAllAsRead} className="text-sm text-purple-600 hover:underline font-medium">
            Mark all as read
          </button>
        )}
      </div>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl mb-6 w-fit">
        {["all", "unread", "read"].map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition capitalize ${filter === f ? "bg-white text-purple-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
          >
            {f}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((notif) => (
            ...
            >
              <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${notif.is_read ? "bg-gray-100 text-gray-400" : "bg-purple-100 text-purple-600"}`}>
              ...
              {!notif.is_read && <div className="w-2.5 h-2.5 rounded-full bg-purple-500 flex-shrink-0 mt-1" />}
            </div>
          ))}
        </div>
      )}
